...
class SAMReader():
    def __init__(self, 
        ...
        rospy.wait_for_service(ns+'create_reader')
        create_reader = rospy.ServiceProxy(ns+'create_reader', CreateReader)
        ...
        while success == False:
            ...
            if len(sp.properties.name) > 0:
                ...
                rp = ReaderProperties(slot_name, agent_name)                               
                r = create_reader(rp)
                success = r.success
                if success == True:
                    type_class = roslib.message.get_message_class(type_str)
                    self._sub = rospy.Subscriber(r.topic_name, 
                                                 type_class,
                                                 reader_cb)
                else:
                    if len(agent_name) == 0:
                        rospy.logwarn("Could not create SAM Reader for Slot '%s'." 
                                      " Retrying...",slot_name)
                    else:
                        rospy.logwarn("Could not create SAM Reader for Slot '%s'" 
                ...
                if len(agent_name) == 0:
                    rospy.logerr("Could not create SAM Reader for Slot '%s'. The Slot does not exist."
                                 " Retrying...",slot_name)
                else:
                    rospy.logerr("Could not create SAM Reader for Slot '%s'. The Slot does not exist"
                                 " ('%s'). Retrying...",slot_name, agent_name)
